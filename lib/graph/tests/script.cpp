...
TEST_CASE("Script using 'this'")
{
    auto g = new Graph();
    auto a = new ScriptNode("a", g);
    a->setScript(
      "input('x', float, 123.0)\n"
      "input('fromself', float)\n"
      "input('expr', float)\n"
      "output('showfromself', fromself)"
      );
    a->getDatum("fromself")->setText("this.x");
    a->getDatum("expr")->setText("this.x + 1");
    ...
    // sanity of 'this' datums
    {
      auto fromself = a->getDatum("fromself");
      REQUIRE(fromself != NULL);
    ...
    SECTION("use of 'this'")
    {
      ...
      REQUIRE(PyFloat_AsDouble(a->getDatum("expr")->currentValue()) == 124.0);
      REQUIRE(PyFloat_AsDouble(a->getDatum("showfromself")->currentValue()) == 123.0);
    }

    SECTION("change datum")
    {
    a->getDatum("x")->setText("231.0");
    REQUIRE(PyFloat_AsDouble(a->getDatum("fromself")->currentValue()) == 231.0);
    }
