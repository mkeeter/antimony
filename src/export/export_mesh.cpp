...
void ExportMeshWorker::run()
{
    if (resolution == -1)
    {
        auto resolution_dialog = new ResolutionDialog(
                &shape, RESOLUTION_DIALOG_3D, false);
        if (!resolution_dialog->exec())
            return;
        resolution = resolution_dialog->getResolution();
        detect_features = resolution_dialog->getDetectFeatures();
    ...
    if (!QFileInfo(QFileInfo(filename).path()).isWritable())
    {
    ...
    auto task = new ExportMeshTask(
            shape, bounds, resolution, detect_features, filename);
